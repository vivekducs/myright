...
import { DIGITAL_GUIDEBOOK_PAGES } from '../data/guidebookData';
...
export const DigitalGuidebook: React.FC<DigitalGuidebookProps> = ({
  ...
  const totalPages = DIGITAL_GUIDEBOOK_PAGES.length;

  const currentBookPage: GuidebookPage =
    DIGITAL_GUIDEBOOK_PAGES.find((p) => p.page_number === currentPage) ||
    DIGITAL_GUIDEBOOK_PAGES[0];

  const filteredPages = DIGITAL_GUIDEBOOK_PAGES.filter((p) => {
    const matchesSearch =
      searchQuery.trim() === '' ||
      p.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      p.chapter.toLowerCase().includes(searchQuery.toLowerCase()) ||
      p.situation_trigger.toLowerCase().includes(searchQuery.toLowerCase()) ||
      p.your_rights_summary.some((r) => r.toLowerCase().includes(searchQuery.toLowerCase())) ||
      p.statutory_provisions.some((sp) =>
        `${sp.code} ${sp.section_or_article} ${sp.official_title}`
          .toLowerCase()
          .includes(searchQuery.toLowerCase())
      );
    const matchesChapter =
      selectedChapterFilter === 'all' || p.chapter.includes(selectedChapterFilter);
  ...
  return (
    ...
    >
      {/* Book Top Ribbon / Official Header */}
      <div className="bg-[#1A3841] text-[#FFF3C8] rounded-3xl p-5 sm:p-7 border-2 border-[#E5CB90]/60 shadow-xl relative overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-[#34A99D] font-bold hidden sm:inline">
                  • BNSS 2023 / BNS 2023 / BSA 2023 / Constitution
                </span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-[#E5CB90]/90 font-medium mt-1">
                A 30-second situational manual: <span className="text-white font-bold">Situation → Your Rights → What To Do → Where To Complain</span>
              </p>
            </div>
          </div>
        ...
        {/* Search and Table of Contents Bar */}
        <div className="mt-6 pt-5 border-t border-white/15 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
            <span className="text-xs text-[#E5CB90] font-bold shrink-0">Jump Page:</span>
            <select
              value={currentPage}
              onChange={(e) => setCurrentPage(Number(e.target.value))}
              className="px-3 py-1.5 rounded-full bg-[#FFF3C8] text-[#1A3841] text-xs font-bold focus:outline-none cursor-pointer border border-[#E5CB90]"
            >
              {DIGITAL_GUIDEBOOK_PAGES.map((page) => (
      ...
      <div className="relative bg-white rounded-3xl border-3 border-[#E5CB90] shadow-2xl overflow-hidden transition-all">
        ...
        <div className="bg-[#1A3841] text-[#FFF3C8] px-6 sm:px-8 py-3.5 flex items-center justify-between border-b-2 border-[#E5CB90]/40">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-[#E5CB90]/90">
              Page {currentPage} of {totalPages}
            </span>
          </div>
        </div>

        {/* --- Page Content Layout: 4-Step Framework --- */}
        <div className="p-6 sm:p-8 lg:p-10 space-y-8">
          
          {/* Page Title & Statutory Shield Header */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b-2 border-[#E5CB90]/40">
            ...
            {/* Official Statutory Provisions Badges */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* STEP 2: Your Constitutional & Statutory Rights */}
              ...
              <div>
                ...
                <ul className="space-y-2.5">
                  {currentBookPage.your_rights_summary.map((right, rIdx) => (
                    <li key={rIdx} className="flex items-start gap-2.5 text-xs sm:text-sm font-medium text-[#1A3841]">
            ...
            {/* STEP 3: Immediate What To Do (Under 30 Seconds) */}
              ...
              <div>
                ...
                <ul className="space-y-2.5">
                  {currentBookPage.immediate_action_steps.map((action, aIdx) => (
                    <li key={aIdx} className="flex items-start gap-2.5 text-xs sm:text-sm font-medium text-[#1A3841]">
            ...
            {/* STEP 4: Where To Complain & Official Escalation Forums */}
          ...
          {/* --- What To Say Script (Direct Spoken Words) --- */}
          <div className="p-6 rounded-3xl bg-[#1A3841] text-[#FFF3C8] border-2 border-[#E5CB90] shadow-md">
            <div className="flex items-center justify-between gap-4 mb-3">
              ...
              <button
                ...
                className="px-3 py-1 rounded-full bg-white/10 hover:bg-white/20 text-[11px] font-bold text-white transition-all cursor-pointer"
              >
        ...
        {/* --- Page Navigation Footer / Book Controls --- */}
        <div className="bg-[#FFF3C8] px-6 sm:px-8 py-5 border-t-2 border-[#E5CB90] flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-full text-xs font-black transition-all cursor-pointer shadow-md ${
              currentPage === 1
              ...
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all cursor-pointer ${
                  page.page_number === currentPage
                    ? 'w-7 bg-[#34A99D] shadow-xs'
                    : 'bg-[#1A3841]/20 hover:bg-[#1A3841]/50'
                }`}
      ...
      {/* --- Table of All 11 Digital Guidebook Chapters --- */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-display text-xl font-bold text-[#1A3841]">
                Complete Guidebook Chapters (11 Core Modules)
              </h3>
              <p className="text-xs text-[#458393] font-semibold">
                Click any chapter to jump directly into the full statutory breakdown
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
          {DIGITAL_GUIDEBOOK_PAGES.map((page) => (
            <button
              ...
              className={`p-4 rounded-2xl text-left border-2 transition-all cursor-pointer flex items-start gap-3 ${
                page.page_number === currentPage
                  ? 'bg-[#1A3841] text-[#FFF3C8] border-[#34A99D] shadow-md scale-[1.02]'
            ...
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 font-black text-xs ${
                page.page_number === currentPage ? 'bg-[#34A99D] text-white' : 'bg-[#E5CB90] text-[#1A3841]'
              }`}>
                {page.page_number}
              </div>
              <div className="min-w-0">
                <span className={`text-[10px] font-black uppercase tracking-wider block ${
                  page.page_number === currentPage ? 'text-[#E5CB90]' : 'text-[#458393]'
                }`}>
                ...
                <span className="text-[10px] opacity-75 line-clamp-1 block mt-0.5">
